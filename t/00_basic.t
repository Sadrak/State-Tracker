...
subtest "toggle", sub {
    ...
    ok($tracker->toggle("$flag1|$flag1")); ok($tracker == 1);
    ok($tracker->toggle("$flag1|$flag2")); ok($tracker == 2);
};
...
subtest "counter", sub {
    ...
    ok($tracker == 0);
    is($tracker->counter($flag1), undef);
    ...
    is($tracker->increment("$flag1|$flag2"), 2); ok($tracker == 3); is($tracker->counter($flag1), 5);

    is($tracker->decrement($flag1, 5), 5); ok($tracker == 2); is($tracker->counter($flag1), 0);
